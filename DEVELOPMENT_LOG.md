...
#### 89) P2 继续推进：客户侧预约管理页落地（前台可用流程）
- 本轮继续沿 token 安全链推进，把“API 可用”进一步升级成“前台流程可用”。
- 本轮覆盖：
  - `src/components/site/BookingManagePanel.tsx`
  - `src/app/[locale]/booking/manage/[token]/page.tsx`
- 已完成：
  - 新增客户侧预约管理组件 `BookingManagePanel`
    - 展示客户姓名、服务、日期、时间、状态
    - 提供“取消预约”按钮
    - 提供“改约”表单（日期 / 时间 / 备注）
    - 直接调用 token PATCH 接口执行安全操作
  - 新增前台页面 `/[locale]/booking/manage/[token]`
    - 通过 token 加载预约信息
    - 使用与官网一致的 Header / Footer / SectionShell
    - 让 token 能力首次真正出现在客户可访问的前台流程中
  - 构建修复：
    - 修正新页面相对路径错误
    - 重新 build 验证通过
- 本轮价值：
  - token 链路已经不再只是后台和接口能力，而是开始成为真实客户流程；
  - 项目已具备“客户通过安全链接自助改约 / 取消”的产品雏形；
  - 这是预约安全闭环从基础设施走向用户可见流程的关键一步。
...
- 不只扩展接口；
- 已开始补前台客户使用入口；
- 让安全 token 体系真正连接到站点产品层。

### 下一步建议
1. 为客户侧 token 流程补通知与日志，提升可追踪性。
2. 继续优化客户管理页的文案、状态反馈与边界处理。
3. 让后台详情页与客户侧管理页形成更完整的运营闭环。

#### 90) 客户侧预约管理流程继续收口：状态同步 + 边界处理
- 本轮继续沿客户侧 token 流程推进，把前台预约管理页从“能操作”提升到“更可用、更像正式流程”。
- 本轮覆盖：
  - `src/components/site/BookingManagePanel.tsx`
- 已完成：
  - 操作后即时状态同步：
    - token PATCH 成功后，前端会立即更新当前预约的日期、时间、状态；
    - 改约后页面会直接显示新的预约时间；
    - 取消后页面会直接显示 `CANCELLED` 状态。
  - 边界状态处理：
    - 若当前预约已 `CANCELLED` 或 `COMPLETED`，前端会显示“不可再在线修改”的提示；
    - 同时禁用取消按钮与改约表单输入，避免继续误操作。
  - 客户侧可用性增强：
    - 预约详情展示改为跟随最新状态刷新，而不是停留在初始加载值；
    - 让客户管理页更接近正式自助流程，而不只是接口演示页面。
- 本轮价值：
  - 客户侧管理页不再只是“能发请求”，而是开始具备真实交互闭环；
  - 边界约束和即时反馈能显著减少误操作与理解成本；
  - 这一步让预约安全 token 流程从“可用”走向“更可靠”。
...
- 不改接口结构；
- 重点收口前台交互体验与边界处理；
- 让 token 管理链更接近正式可交付的用户流程。

### 下一步建议
1. 为 token 操作补通知与日志，提升可追踪性与运营闭环。
2. 继续收口客户侧文案与状态提示，让取消 / 改约体验更清晰。
3. 让后台预约详情页与客户管理页之间形成更完整的协同视图。
  - 登录页 / 登录表单
  - Dashboard
  - 预约列表 / 预约详情
  - 服务列表 / 新建服务 / 编辑服务
...
#### 67) 后台中文化与工作台表达收口（第一批批量修正）
- 本轮不再按“一两个点”零碎修，而是按一个完整阶段推进：
  - 目标：批量收口后台高频页面里的英文语感与开发期表达，让整体更像正式运营后台。
- 本轮覆盖页面：
...
- 已完成：
  - Dashboard / 后台首页
    - `dashboard MVP` 收口为更自然的 `后台首页雏形`
    - `Workspace` eyebrow 在中文模式下收口为 `工作台`
    - 内容管理卡片文案从 `Hero / gallery` 语感收口为 `首页主视觉 / 图片资料`
    - 侧栏定位卡片标题与说明继续中文化
  - 内容管理页
    - `返回 Dashboard` → `返回后台首页`
    - `内容工作台 / Content workspace` 收口为 `内容工作区`
    - 页面描述中的 `Hero / FAQ` 收口为 `首页主视觉 / 常见问题`
    - `Hero 预览 / 当前 Hero 图片 / 当前还没有 Hero 图片` 收口为 `主视觉预览 / 当前主视觉图片 / 当前还没有主视觉图片`
    - 空状态描述也同步中文化
  - 服务管理页
    - `返回 Dashboard` → `返回后台首页`
    - `服务库` → `服务资料库`
    - 描述中的 `管理节奏` 收口为更自然的 `管理体验`
  - 系统设置页
    - `运营配置` → `运营配置项`
    - `基础系统配置` → `基础系统设置`
    - 描述中的 `站点基础信息` 与其他页面统一为 `站点基本信息`
- 本轮价值：
  - 这是一次页面级批量收口，而不是零碎打补丁；
  - 后台首页、内容页、服务页、设置页四条高频路径的中文表达同时前进了一步；
  - 后台整体观感更一致，也更接近真实交付给店主/运营的系统。
...
- 不改接口；
- 不改业务逻辑；
- 重点做后台中文化与工作台表达的第一批批量修正。

### 下一步建议
1. 继续做第二批批量收口，优先 appointments / gallery / settings 侧栏与说明区块。
2. 继续把 `ContentEditor` 里剩余能中文化的 `Hero / Gallery` 表达继续收掉。
3. 继续把 `ContentEditor` 中剩余区块逐步收进统一区块卡片模式。
...
#### 71) 后台中文化与表单体系收口（第五批批量修正）
- 本轮继续按阶段推进，重点从 `ContentEditor` 扩展到 `ServiceForm`，正式开始收口后台“表单体系”的可读性。
- 本轮覆盖：
  - `src/components/admin/ServiceForm.tsx`
  - 并同步检查后台高频页面文案残留
- 已完成：
  - `ServiceForm`
    - 接入共享 `NoticePill`，保存结果不再是普通灰字，而是进入统一 success / error 反馈体系；
    - `Slug 标识` → `链接标识`，并补充说明：建议使用小写字母、数字和连字符；
    - `排序` → `显示顺序`，并补充说明：数字越小越靠前；
    - `摘要（德语/英语）` → `简短介绍（德语/英语）`，并补充用途说明；
    - `描述（德语/英语）` → `详细介绍（德语/英语）`，并补充更自然的内容说明；
    - `时长（分钟）` → `服务时长（分钟）`，补充示例；
    - `价格 (€)` → `服务价格 (€)`，补充示例；
    - `精选` → `设为推荐服务`；
    - `上架` → `上架显示`；
    - 底部操作区也统一为更稳定的 `flex-wrap + NoticePill` 结构。
- 本轮价值：
  - 后台不再只是“页面标题中文”，而是开始让服务表单本身也更像给店主/运营填写的表单；
  - `ServiceForm` 现在的字段语义、辅助提示和反馈样式都明显比之前更接近正式后台产品；
  - 后台“表单体系中文化”这条线正式启动，而不再只停留在内容页。
...
- 不改接口；
- 不改业务逻辑；
- 重点推进后台服务表单的字段语义、示例提示和反馈体系统一。

### 下一步建议
1. 继续收口 `ServiceForm` 之外的表单组件（如新建/编辑服务页面周边说明、其它输入提示）。
2. 继续清后台全局仍偏英文思维的说明句式，优先 dashboard / gallery / content 侧栏卡片。
3. 继续检查后台高频术语与字段词，进一步统一成中文后台的自然表达。

#### 72) 后台功能完善：服务管理录入体验增强
- 本轮不只做中文化，而是明确补一批后台体验能力，重点提升服务管理表单的录入效率与可用性。
- 本轮覆盖：
  - `src/components/admin/ServiceForm.tsx`
- 已完成：
  - 自动生成链接标识（slug）
    - 当管理员填写服务名称时，会自动按英文名优先、德文名兜底生成 slug；
    - 只要用户没有手动改过 slug，就会持续自动同步；
    - 一旦用户手动编辑 slug，就停止自动覆盖，避免干扰。
  - 增加链接预览
    - 在“链接标识”字段下方新增实时路径预览：
      - 例如 `/services/hot-stone-massage`
    - 可以在保存前直观看到最终链接效果。
  - 数值输入更适合录入
    - `显示顺序` 改为 number 输入
    - `服务时长` 改为 number 输入，并限制步长为 5
    - `服务价格` 改为 number 输入，并支持小数步长 `0.01`
  - 反馈体验继续统一
    - `ServiceForm` 接入共享 `NoticePill`
    - 保存成功 / 失败提示进入统一 success / error 反馈体系
    - 底部操作区改为更稳的 `flex-wrap + NoticePill`
- 本轮价值：
  - 服务管理页不再只是“字段翻译更自然”，而是实际录入体验更顺手；
  - 自动 slug + 路径预览可以减少人工输错和来回修改；
  - 数值字段改成更合适的输入类型后，后台日常维护效率会明显更高。

### 本阶段验证追加
- `npm run build` 已通过。

### 本阶段结论
这一轮属于“后台功能完善 + 体验增强”阶段：
- 不改接口；
- 不改核心业务逻辑；
- 但明显提升了服务管理表单的录入效率和交互体验。

### 下一步建议
1. 把类似的录入体验增强继续扩展到其它后台表单（如 ContentEditor / settings 中适合自动化或数字化输入的字段）。
2. 继续清后台全局仍偏英文思维的说明句式，优先 dashboard / gallery / content 侧栏卡片。
3. 继续推进后台“功能完善”线，而不只做文案：优先考虑服务列表筛选、批量操作或内容页更直观的预览/约束提示。

#### 73) 后台功能完善：服务列表筛选增强
- 本轮继续沿着“服务管理体验增强”这条线推进，不再只做表单，而是补服务列表页的日常运营效率。
- 本轮覆盖：
  - `src/app/admin/services/page.tsx`
- 已完成：
  - 新增服务列表快速筛选：
  ...
  - 通过 URL 查询参数驱动筛选：
    - `/admin/services`
    - `/admin/services?filter=active`
    - `/admin/services?filter=inactive`
    - `/admin/services?filter=featured`
  - 空状态适配当前筛选结果：
    - 不再只提示“没有服务数据”
    - 会根据当前筛选语境提示“当前筛选下没有服务数据”
  - 列表描述同步更新：
    - 明确说明现在支持按状态快速筛选，减少来回翻找
- 本轮价值：
  - 服务列表从“只能全量看”升级为“可快速切视图”
  - 这更符合后台日常维护场景，比如：
    - 只看已上架服务
    - 检查哪些服务还没上架
    - 快速巡检推荐服务
...
### 下一步建议
1. 继续做服务管理体验增强，优先考虑批量操作或更直观的状态/推荐筛选组合。
2. 把类似的筛选体验扩展到图片管理或内容管理。
3. 继续推进后台“功能完善 + 中文化”双线并行，而不是只修文案。

#### 74) 后台功能完善：图库管理体验增强
- 本轮继续沿“后台巡检效率”这条线推进，把同类能力从服务管理扩展到图库管理。
- 本轮覆盖：
  - `src/app/admin/gallery/page.tsx`
- 已完成：
  - 新增筛选结果总览提示：
    - 当前筛选结果多少张
    - 全部图片总共有多少张
  - 让管理员进入图库页后，不用自己数列表就能立刻知道：
    - 当前视图筛出了多少
    - 库里总量是多少
  - 空状态继续增强：
    - 如果当前是“全部”视图为空，就直接引导去上传第一张图库图片；
    - 如果是某个筛选视图为空，就提示切换筛选条件或去内容管理页上传图片。
  - 图库页主说明同步更新：
    - 明确说明现在支持按启用状态、封面和来源快速切换巡检视图。
- 本轮价值：
  - 图库页从“能筛选”升级为“更适合巡检”
  - 管理员现在可以更快判断：
    - 当前筛选有没有结果
...
- 不改接口；
- 不改数据结构；
- 但明显提升了图库管理页的巡检效率和信息感知能力。

### 下一步建议
1. 继续增强图库管理，例如加入更多状态统计或更直观的组合筛选。
2. 把类似的“筛选结果 + 总量感知”体验扩展到预约管理或内容管理。
3. 继续沿“后台功能完善 + 中文化”双线并行推进，而不是只做表层修改。

#### 75) 后台功能完善：预约管理体验增强
- 本轮继续沿“后台巡检效率”这条线推进，把“结果感知 + 空状态分流”的体验扩展到预约管理页。
- 本轮覆盖：
  - `src/app/admin/appointments/page.tsx`
- 已完成：
  - 新增预约列表结果感知：
  ...
  - 这样管理员进入预约页后，能立刻知道：
    ...
    - 当前页的展示上限是多少
  - 空状态按筛选语境拆分：
    - `全部` 视图为空：提示当前还没有预约数据，或运行环境未连库
    - 某个状态视图为空：提示当前筛选下没有符合条件的预约，可切换状态或等待新预约进入
  - 列表说明同步增强：
    - 明确说明现在也能更直观看到当前筛选结果数量
- 本轮价值：
  - 预约页从“能筛选”升级为“更适合巡检和快速判断”
  - 管理员现在不用盯着整表估计数据量，进入页面就能知道当前视图的结果规模
  - 这与服务列表、图库页一起，开始形成更一致的后台管理体验模式。
...
- 不改接口；
- 不改数据结构；
- 但明显提升了预约管理页的结果感知与巡检效率。

### 下一步建议
1. 把这种“筛选结果 + 总量感知”模式继续扩展到内容管理或其它高频后台页面。
2. 继续增强预约管理，例如补充更直观的状态统计或高频视图切换。
3. 继续沿“后台功能完善 + 中文化”双线并行推进，而不是只做局部优化。

#### 76) 后台功能完善：三条线综合增强（内容 / 服务 / 图库）
- 本轮不再拆小阶段，而是按用户要求把 3 个方向一起处理，做一次综合增强交付。
- 本轮覆盖：
  - `src/app/admin/content/page.tsx`
  - `src/app/admin/services/page.tsx`
  - `src/app/admin/gallery/page.tsx`
- 已完成：
  - 内容管理页：
    - 在内容工作区顶部增加内容状态感知标签：
      ...
      - 启用 FAQ 数量
      - 图库图片数量
    - 让内容页不再只是长表单，而是进入页面就能先看到核心内容状态。
  - 服务管理页：
    - 增加“当前筛选结果多少项”
    - 增加“全部服务总量多少项”
    - 与已有筛选组合后，服务页现在具备更完整的结果感知。
  - 图库管理页：
    - 在已有“当前筛选结果 / 全部图片总量”基础上，再增加：
      - 封面图片数量
      - 本地上传数量
    - 让图库页的巡检视角更完整，更接近真正的媒体运营页。
- 本轮价值：
  - 内容、服务、图库三条线现在都不再只是“能编辑”，而是更像“进入页面先看状态，再决定操作”的运营后台。
  - 后台体验开始形成更统一的产品模式：
    - 先看状态
    - 再做筛选
    - 再进入编辑/处理
  - 这比单点修文案价值更高，也更符合“多处理几个问题、按阶段推进”的要求。
...
- 不改接口；
- 不改数据结构；
- 但同时提升了内容管理、服务管理、图库管理三块页面的状态感知与巡检效率。

### 下一步建议
1. 下一轮可以继续做“多方向综合阶段”，把预约管理也并入统一的状态看板模式。
2. 可继续增强内容页，让 Hero / FAQ / 图库区块各自带更明确的局部统计或预览提示。
3. 服务页后续可继续补组合筛选、状态统计或批量操作入口，进一步向真正运营后台靠拢。

#### 77) 后台功能完善：四条线综合增强（预约 / 内容 / 服务 / 图库）
- 本轮继续按综合阶段推进，把预约也并入统一的状态看板与结果感知模式。
- 本轮覆盖：
  - `src/app/admin/appointments/page.tsx`
  - `src/app/admin/content/page.tsx`
  - `src/app/admin/services/page.tsx`
- 已完成：
  - 预约管理页：
    - 改为先取全部预约，再在页面内做状态筛选；
    - 新增“全部预约总量”提示；
    - 新增状态统计卡：待确认 / 已确认 / 已完成 / 已取消；
    - 让预约页从“能筛选”进一步升级为“能先看状态分布再处理”。
  - 内容管理页：
    - 在原有状态标签基础上，继续补充“封面图片数量”提示；
    - 让内容页对 Hero / 图库状态的感知更完整。
  - 服务管理页：
    - 在服务列表区域新增已上架数量、推荐服务数量提示；
    - 补齐服务页的局部状态看板能力。
- 本轮价值：
  - 预约、内容、服务三页进一步统一到“先看状态 / 再筛选 / 再操作”的后台模式；
  - 与图库页已有的统计提示一起，后台正在形成一致的运营工作流；
  - 这比单点补文案或单页修饰更接近真正可运营后台的方向。
...
- 不改接口；
- 不改数据结构；
- 但继续强化了预约、内容、服务页面的状态感知与看板化体验。

### 下一步建议
1. 继续沿四条线综合推进，把更多局部统计、组合筛选和高频操作入口补齐。
2. 内容页后续可继续增强各区块的局部统计/预览提示。
3. 服务与预约页可继续补更细的状态分布或组合筛选，进一步提升巡检效率。

#### 78) 后台功能完善：三条线继续综合增强（内容 / 服务 / 图库）
- 本轮按用户要求，聚焦 3 个方向一起推进：内容管理、服务管理、图库管理，不再拆成小修小补。
- 本轮覆盖：
  - `src/app/admin/content/page.tsx`
  - `src/app/admin/services/page.tsx`
  - `src/app/admin/gallery/page.tsx`
- 已完成：
  - 内容管理页：
    - 顶部状态标签新增 `FAQ 总数`；
    - 与已有的 `启用 FAQ`、`图库图片`、`封面图片` 组合后，内容页顶部状态感知更完整。
  - 服务管理页：
    - 在列表区域局部状态条里新增 `未上架数量`；
    - 现在服务页可同时感知：已上架、未上架、推荐服务 三类核心状态。
  - 图库管理页：
    - 在顶部巡检标签里新增 `启用图片数量`；
    - 现在图库页顶部可同时看到：筛选结果、总量、封面数量、启用数量、本地上传数量。
- 本轮价值：
  - 三个高频后台页面继续朝统一运营模式收口；
  - 不只是“能筛选”，而是“进入页面就能先判断当前状态分布”；
  - 这更符合后台巡检与日常运营的真实使用场景。
...
- 不改接口；
- 不改数据结构；
- 但继续提升了内容页、服务页、图库页的状态感知完整度。

### 下一步建议
1. 继续沿 3 个方向一起推进，补更细的组合筛选和局部统计。
2. 内容页可继续增强 Hero / FAQ / 图库三个区块的局部预览提示。
3. 服务与图库页可继续向批量操作或更强的巡检视图靠拢。

#### 79) 后台功能完善：向正式运营系统推进（后台首页 / 内容 / 服务 / 图库）
- 本轮目标从“继续增强看板”升级为“更接近正式运营系统”，重点补运营入口与页面联动，而不只是继续堆统计数字。
- 本轮覆盖：
  - `src/app/admin/page.tsx`
  - `src/app/admin/content/page.tsx`
  - `src/app/admin/services/page.tsx`
...
- 已完成：
  - 后台首页：
    ...
    - 提供直达入口：
      ...
      - 内容维护
      - 封面图片巡检
    - 让首页从“概览页”进一步升级为“运营入口页”。
  - 服务管理页：
    - 新增“服务管理快捷入口”卡片；
    - 可直接进入：未上架服务、推荐服务、新建服务；
    - 让服务页不仅能巡检，也能更快进入高频动作。
  - 图库管理页：
    - 新增“图库快捷入口”卡片；
    - 可直接进入：封面图片、本地上传图片、内容页继续编辑；
    - 让图库页具备更明确的运营动作入口。
  - 内容管理页：
    - 新增“内容管理快捷入口”卡片；
    - 把 Hero 维护、FAQ/营业时间维护、前往图库巡检这类高频动作收成统一入口；
    - 让内容页不再只是长表单，而更像内容运营工作区。
- 本轮价值：
  - 后台首页、内容页、服务页、图库页开始形成“看状态 + 走快捷入口 + 再进入处理”的正式运营节奏；
  - 后台正在从“可管理”继续往“可运营”推进；
  - 这比继续零散补数字更接近正式系统所需的产品感。
...
- 不改接口；
- 不改数据结构；
- 但明显提升了后台首页与三大模块的运营入口能力和页面联动感。

### 下一步建议
1. 继续往“正式运营系统”推进，下一轮可补更多高频处理入口与跨页联动。
2. 服务与图库页后续可继续加入批量操作或更明确的任务型入口。
3. 后台首页后续可继续收敛成更完整的运营总控台。

#### 80) 后台功能完善：后台首页 + 预约管理任务入口增强
- 本轮继续沿“正式运营系统”方向推进，重点把后台首页与预约管理页往任务型后台再推一层。
- 本轮覆盖：
  - `src/app/admin/page.tsx`
  - `src/app/admin/appointments/page.tsx`
- 已完成：
  - 后台首页：
    - 在“今日优先处理”卡片里新增“跟进已确认预约”入口；
    - 让首页不只处理 pending，也能继续承接 confirmed 预约的后续运营动作。
  - 预约管理页：
    - 把原来的状态统计区域升级为“状态统计 + 快捷处理入口”组合布局；
    - 新增预约处理快捷入口：
      - 处理待确认预约
      - 查看已确认预约
      - 查看已完成预约
    - 让预约页从“看列表和改状态”继续升级为“能先看状态分布，再直接进入任务处理”。
- 本轮价值：
  - 后台首页与预约页的联动更强，首页上的任务入口与预约页内部的处理入口已经开始形成上下游关系；
  - 预约模块开始具备更像正式运营系统的任务流，而不是单纯一张数据表；
  - 这一步进一步强化了“先看状态、再走入口、再处理任务”的后台产品模式。
...
- 不改接口；
- 不改数据结构；
- 但进一步提升了后台首页与预约管理页的任务入口能力和处理流感。

### 下一步建议
1. 继续收口后台首页，让它更像统一运营总控台。
2. 预约模块后续可继续补更强的任务分组、高频处理路径或更直观的状态分布展示。
3. 与服务 / 内容 / 图库页的快捷入口继续联动，进一步靠近正式运营系统。

#### 81) 后台功能完善：后台首页总控台 + 服务/图库任务入口继续增强
- 本轮继续沿“正式运营系统”方向推进，重点补后台首页的任务感，以及服务页、图库页的待处理入口完整度。
- 本轮覆盖：
  - `src/app/admin/page.tsx`
  - `src/app/admin/services/page.tsx`
  - `src/app/admin/gallery/page.tsx`
- 已完成：
  - 后台首页：
    - 在“今日优先处理”卡片顶部增加任务分组标签：预约 / 服务 / 内容 / 图片；
    - 让首页的优先处理区更像总控台，而不是单纯的一组链接。
  - 服务管理页：
    - 在快捷入口中新增“巡检已上架服务”；
    - 现在服务页的任务入口已覆盖：已上架、未上架、推荐服务、新建服务。
  - 图库管理页：
    - 在快捷入口中新增“巡检启用中图片”；
    - 现在图库页的任务入口已覆盖：封面图片、本地上传、启用图片、返回内容页继续编辑。
- 本轮价值：
  - 后台首页的任务感继续增强，更接近总控台；
  - 服务页、图库页的快捷入口逐渐从“少量入口”进化为“更完整的待处理视图入口”；
  - 后台整体进一步接近“正式运营系统”而不是普通管理页集合。
...
- 不改接口；
- 不改数据结构；
- 但继续提升了后台首页、服务页、图库页的任务入口完整度与总控感。

### 下一步建议
1. 继续把后台首页收口成统一运营总控台。
2. 服务与图库页后续可继续补更明确的任务分组或批量处理入口。
3. 后台各模块的快捷入口继续联动，进一步靠近正式运营系统。

#### 82) 后台功能完善：后台首页总控台 + 内容管理任务面板继续增强
- 本轮继续沿“正式运营系统”方向推进，重点增强后台首页的处理顺序感，以及内容管理页的侧栏任务感。
- 本轮覆盖：
  - `src/app/admin/page.tsx`
  - `src/app/admin/content/page.tsx`
- 已完成：
  ...
  - 内容管理页：
    ...
    - 将高频判断项提炼为更像任务面板的状态：
      ...
      - FAQ 启用数量 / 总量
      - 图库封面数量
    - 让内容页从“内容工作区 + 快捷入口”继续升级为“工作区 + 任务面板 + 状态判断”。
- 本轮价值：
  - 后台首页的运营感继续增强，更接近正式运营系统中的总控台；
  - 内容管理页不再只是大表单，也不只是状态展示，而开始具备真正的内容待办视角；
  - 这让后台更接近“先判断、再处理”的正式运营工作流。
...
- 不改接口；
- 不改数据结构；
- 但继续提升了后台首页与内容管理页的任务面板能力和正式运营感。

### 下一步建议
1. 继续把后台首页收口成统一运营总控台。
2. 服务与图库页后续可继续补更明确的任务分组、批量处理入口或优先动作入口。
3. 后台各模块继续朝“工作区 + 状态判断 + 快捷任务”模式统一。

#### 83) 结合 SEO + 安全设计方案：后台信息架构先落地（优先后台）
- 本轮开始把外部提供的“SEO + 安全设计增强版方案”真正映射到当前项目里，优先落到后台信息架构，而不是只停留在讨论层。
- 本轮覆盖：
  - `src/app/admin/page.tsx`
  - `src/app/admin/settings/page.tsx`
  - `ROADMAP.md`
...
- 已完成：
  - 后台首页：
    - 在快速开始卡片中新增“系统与安全”入口；
    - 在“今日优先处理”区增加“安全”分组标签；
    - 新增“检查系统与安全设置”快捷入口；
    - 让后台首页开始显式承接安全/系统治理入口。
  - 系统设置页：
    - 新增“功能开关”面板；
    - 明确展示当前项目建议的默认开启 / 默认关闭能力；
    - 新增“SEO / 隐私待办”面板；
    - 开始把 Impressum / Datenschutzerklärung、隐私同意、数据保留/删除、SEO 与安全平衡等要求，先作为后台的可见运营事项表达出来。
  - 项目文档：
    - `ROADMAP.md` 已新增：`安全设置 / 功能开关 / 合规待办后台化`
    - `PROJECT_STATUS.md` 已把系统设置页、安全设置、功能开关、SEO/隐私待办纳入建议完成项
- 本轮价值：
  - 当前项目正式把“SEO + 安全 + 合规”纳入后台主线，而不是只围绕预约、内容和图片做页面体验优化；
  - 后台首页和设置页开始具备更像正式运营系统的治理能力表达；
  - 为后续真正实现防刷、隐私同意、功能开关、安全日志等能力铺好了后台产品层结构。
...
- 不改接口；
- 不改数据结构；
- 但已把 SEO / 安全 / 合规能力正式纳入后台信息架构与项目路线图。

### 下一步建议
1. 继续优先做后台，把安全设置、功能开关、合规待办逐步从“展示层”推进到“可配置能力”。
2. 结合预约防刷与后台安全目标，后续优先推进验证码、防滥用、隐私同意等可落地项。
3. 同步把 SEO 设置页与合规页面（Impressum / Datenschutzerklärung）继续产品化，靠近正式运营系统。

#### 84) 三个方向同时推进：后台首页 / 预约管理 / 系统设置继续产品化
- 本轮按用户要求，继续以后台为中心，同时推进 3 个方向：后台首页总控、预约管理、安全/治理设置。
- 本轮覆盖：
  - `src/app/admin/page.tsx`
  - `src/app/admin/appointments/page.tsx`
  - `src/app/admin/settings/page.tsx`
- 已完成：
  - 后台首页：
    - 在“建议处理顺序”上方增加流程标签：
      ...
      - 再内容维护
      - 最后安全巡检
    - 让首页从“有任务顺序”进一步升级为“有运营阶段感”的总控台。
  - 预约管理页：
    - 在右侧快捷处理卡片中新增“防滥用视角”区块；
    - 增加“检查验证码与预约防护”入口，直接联动到系统设置页；
    - 增加“当前高风险待确认预约”表达，开始把预约安全纳入后台视角。
  - 系统设置页：
    - 新增“预约防护 / Booking protection”面板；
    - 把验证码、IP 频率限制、手机号/邮箱频率限制、安全 token、异常预约识别、数据最少化等要求，先正式挂入后台治理面板。
- 本轮价值：
  - 后台首页继续更像正式运营总控台，而不是纯导航页；
  - 预约管理第一次开始显式承接“防刷 / 防恶意预约”视角；
  - 系统设置继续从基础参数页，升级为真正的安全治理与合规治理入口。
...
- 不改接口；
- 不改数据结构；
- 但同时增强了后台首页、预约管理页、系统设置页的治理表达与正式运营感。

### 下一步建议
1. 继续把预约安全相关能力从“后台表达”推进到“真实可配置能力”。
2. 继续把系统设置页做成安全、功能开关、SEO、合规的统一治理中心。
3. 后台首页后续可继续承接更多跨模块的待办与治理任务入口。

#### 85) 三个方向继续同步推进：预约异常视角 / 安全执行清单 / 首页治理入口
- 本轮继续按用户要求，三个方向同步推进，且继续优先后台正式运营系统化。
- 本轮覆盖：
  - `src/app/admin/appointments/page.tsx`
  - `src/app/admin/settings/page.tsx`
  - `src/app/admin/page.tsx`
- 已完成：
  - 预约管理页：
    - 在“防滥用视角”区块中新增“查看爽约记录”入口；
    - 让异常/风险预约视角从 `PENDING` 扩展到 `NO_SHOW` 这类真实经营风险场景。
  - 系统设置页：
    - 新增“安全执行清单 / Security execution checklist”；
    - 将下一批应优先落地的后台安全项整理为 P1~P4：
      - 预约验证码 + 基础频率限制
      - 改约 / 取消安全 token
      - 隐私同意 / 数据保留 / 删除流程
      - 安全日志 / 登录失败记录 / 操作日志增强
    - 让系统设置页开始从治理表达层，走向真正可执行的开发清单层。
  - 后台首页：
    - 新增“查看安全执行清单”入口；
    - 让首页不只是能看到安全治理入口，还能直达下一批应落地的安全任务。
- 本轮价值：
  - 预约页继续引入真实的异常经营视角；
  - 系统设置页继续从“参数页”进化为“安全与治理中心”；
  - 首页继续变成跨模块的运营驾驶舱，而不只是业务模块导航。
...
- 不改接口；
- 不改数据结构；
- 但继续提升了预约管理页、系统设置页、后台首页的治理密度与正式运营感。

### 下一步建议
1. 继续把安全执行清单里的 P1 项（验证码 + 基础频率限制）推进为真正可配置能力。
...
#### 92) 内容管理工作台继续增强：状态感知 + 任务入口 + 区块定位
- 本轮将内容管理页从“长表单编辑页”继续推进为更接近正式运营后台的内容工作台。
- 本轮覆盖：
  - `src/server/services/admin-content.service.ts`
  - `src/app/admin/content/page.tsx`
  - `src/components/admin/ContentEditor.tsx`
- 已完成：
  - 内容统计扩展：
    - 新增 `contactReady`：联系信息是否达到基础可用状态（电话 + 地址）；
    - 新增 `openDayCount`：当前营业日覆盖天数；
    - 新增 `heroCopyReady`：主视觉双语主标题/副标题是否已补齐。
  - 内容页顶部状态增强：
    - 原有 FAQ / 图库 / 封面统计保留；
    - 新增“联系信息已就绪 / 待补充”；
    - 新增“主视觉文案已就绪 / 待完善”；
  ...
  - 快捷入口增强：
    ...
    - 现在可直接定位到：
      ...
      - `#faq-section`
      - `#contact-section`
    - 并结合 ready / missing 状态做即时提示。
  - 内容待办增强：
    - 侧栏新增联系信息完整度；
    - 新增营业时间覆盖度（`openDayCount / 7`）；
    - 让运营更容易一眼判断当前最该补哪里。
  - 编辑区块定位增强：
    - `ContentEditor` 已为主视觉 / 联系信息 / 营业时间 / FAQ / 图库 增加锚点区块；
    - 为后续继续扩展“内容巡检入口”打下结构基础。
- 本轮价值：
  - 内容页从“能改数据”继续升级为“能快速判断优先级并直接进入处理区”；
  - 内容运营体验进一步统一到“先看状态 → 再找入口 → 再编辑”的后台产品模式；
  - 这是低风险但高价值的工作台收口，符合当前项目阶段。
...
- 不改接口协议；
- 不改数据结构；
- 重点强化内容后台的状态感知、任务入口与巡检效率。

### 下一步建议
1. 继续增强内容页的预览能力，让主视觉 / FAQ / 图库更接近“边看边改”。
2. 继续把内容页的待办逻辑和后台首页总控台联动。
3. 若继续推进预约安全线，可开始补更正式的 token 操作通知 / 审计能力。

#### 93) SEO + 安全设计增强版方案正式写入项目文档基线
- 本轮不直接新增业务接口，而是把“德国单店按摩店：SEO + 安全设计增强版方案”正式沉淀为仓库内长期执行文档，避免后续开发继续依赖聊天上下文。
- 本轮新增：
  - `SEO_SECURITY_MVP_PLAN.md`
- 文档定位：
  - 统一项目口径：德国单店按摩店、双语官网、中文后台、轻量单管理员；
  - 同时明确 SEO、预约安全、后台安全、数据安全、通知安全、隐私与合规基础要求；
  - 固定默认功能开关、开发原则、后台产品模式与下一阶段执行优先级。
- 同步更新：
  - `README_CN.md`
    - 接手顺序已纳入 `SEO_SECURITY_MVP_PLAN.md`
    - 明确后续开发应先看该基线文档，而不是只依赖聊天记录
  - `ROADMAP.md`
    - 明确 Phase 5 不只是中文后台，而是逐步形成可运营后台
    - 新增：安全日志 / 操作日志、预约防刷、防恶意预约、隐私同意 / 数据保留 / 删除策略后台化
    - Phase 6 新增 SEO / 安全 / 合规平衡收口项
  - `ARCHITECTURE.md`
    - 明确项目从架构层同时兼顾 SEO、预约安全、数据隐私与后台治理
    - 后台职责扩展到安全入口、合规页面、功能开关、治理中心
    - 安全要求补充：token、自助改约/取消、防刷、审计痕迹、SEO 与安全平衡
  - `HANDOFF.md`
    - 接手顺序与下一阶段重点已纳入 SEO / 安全执行基线
    - 明确后续继续做后台、SEO、安全、合规时应以仓库文档推进
  - `PROJECT_STATUS.md`
    - 项目目标新增“基础安全与隐私治理”
    - 状态说明新增：项目统一方向是 SEO + 预约安全 + 数据隐私 + 中文后台运营并行
    - 风险项新增：安全与隐私治理仍有相当部分停留在表达层
- 本轮价值：
  - 把用户给出的增强版方案从“聊天提示词”升级为“项目执行基线”；
  - 后续开发能更快进入实现，不再反复重建上下文；
  - 有助于把项目更快推进到“完整可运营后台 + 可上线官网”的统一方向。

### 本阶段验证追加
- 本轮为文档与执行基线收口阶段，后续如继续进入代码实现阶段，仍按固定流程执行 `npm run build`。

### 本阶段结论
这一轮属于项目治理基线增强阶段：
- 重点不在新增某一个功能点；
- 而在于把 SEO + 安全 + 合规 + 后台治理要求写入仓库，成为长期可执行约束；
- 这会显著降低下次继续开发的启动成本，并提升后续交付速度。
...
#### 98) 商家邮件通知落地 + 后台自测与合规清理工具
- 本轮聚焦于“交付级”的运营闭环：让店主能实时收到预约通知，并提供配置自测与数据治理工具。
- 本轮覆盖：
...
- 已完成：
  - **商家邮件通知系统**：
    - 新增 `mail.service.ts`，内置双语 HTML 邮件模板；
    - 预约提交 API 异步触发通知，包含客户详情、服务内容与直拨电话链接；
    - 采用非阻塞设计，确保邮件发送失败不影响业务下单。
  - **SMTP 配置自测工具**：
    - 新增测试邮件 API，支持管理员一键验证 SMTP 连通性；
    - 后台设置页增加“发送测试邮件”按钮，提供即时反馈。
  - **合规数据治理工具**：
    - 实现数据清理 API，根据配置的保留期自动物理删除过期预约；
    - 设置页增加“立即执行清理”按钮，满足 GDPR 数据最少化手动触发需求。
  - **架构与安全收口**：
    - 统一修正了深度路由导入路径问题；
    - 所有新增后台接口均强制要求管理员 Session 校验。
- 本轮价值：
  - 店主不再需要盯着后台，预约通过邮件主动触达；
  - 极大降低了 SMTP 联调成本（之前只能通过真实下单测试）；
  - 隐私合规配置（保留期）从展示层进入逻辑实现层。

### 本阶段验证追加
- `npm run build` 已通过。

### 本阶段结论
这一轮完成了 Phase 17 (后台体验增强) 的核心交付：
- 建立了通知链路；
- 补齐了设置页的交互工具；
- 实现了 GDPR 清理逻辑。
