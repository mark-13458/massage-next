    ...
    <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
      <section className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div className="mt-6 space-y-3 text-sm text-stone-700">
          <div className="flex items-center justify-between rounded-2xl border border-stone-200 px-4 py-3"><span>{t.customer}</span><span className="font-medium text-stone-900">{booking.customerName}</span></div>
          <div className="flex items-center justify-between rounded-2xl border border-stone-200 px-4 py-3"><span>{t.service}</span><span className="font-medium text-stone-900">{booking.serviceName}</span></div>
          <div className="flex items-center justify-between rounded-2xl border border-stone-200 px-4 py-3"><span>{t.date}</span><span className="font-medium text-stone-900">{booking.appointmentDate.slice(0, 10)}</span></div>
          <div className="flex items-center justify-between rounded-2xl border border-stone-200 px-4 py-3"><span>{t.time}</span><span className="font-medium text-stone-900">{booking.appointmentTime}</span></div>
          <div className="flex items-center justify-between rounded-2xl border border-stone-200 px-4 py-3"><span>{t.status}</span><span className="font-medium text-stone-900">{booking.status}</span></div>
        </div>
      </section>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-3xl border border-stone-200 p-5">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="rounded-2xl border border-stone-200 px-4 py-3 outline-none focus:border-stone-500" />
              <input type="time" value={time} onChange={(e) => setTime(e.target.value)} className="rounded-2xl border border-stone-200 px-4 py-3 outline-none focus:border-stone-500" />
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder={t.notes} rows={4} className="rounded-2xl border border-stone-200 px-4 py-3 outline-none focus:border-stone-500 sm:col-span-2" />
            </div>
            <button
              type="button"
              disabled={isPending}
