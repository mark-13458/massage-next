    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <input value={hero.eyebrowDe ?? ''} onChange={(e) => setHero({ ...hero, eyebrowDe: e.target.value })} placeholder={localizedFieldPlaceholder(lang, '主视觉上方短标签', 'Top label', 'de')} className="rounded-2xl border border-stone-200 px-4 py-3 text-sm outline-none focus:border-amber-500" />
          <input value={hero.eyebrowEn ?? ''} onChange={(e) => setHero({ ...hero, eyebrowEn: e.target.value })} placeholder={localizedFieldPlaceholder(lang, '主视觉上方短标签', 'Top label', 'en')} className="rounded-2xl border border-stone-200 px-4 py-3 text-sm outline-none focus:border-amber-500" />
          <input value={hero.titleDe ?? ''} onChange={(e) => setHero({ ...hero, titleDe: e.target.value })} placeholder={localizedFieldPlaceholder(lang, '主视觉标题', 'Main title', 'de')} className="rounded-2xl border border-stone-200 px-4 py-3 text-sm outline-none focus:border-amber-500" />
          <input value={hero.titleEn ?? ''} onChange={(e) => setHero({ ...hero, titleEn: e.target.value })} placeholder={localizedFieldPlaceholder(lang, '主视觉标题', 'Main title', 'en')} className="rounded-2xl border border-stone-200 px-4 py-3 text-sm outline-none focus:border-amber-500" />
          <textarea value={hero.subtitleDe ?? ''} onChange={(e) => setHero({ ...hero, subtitleDe: e.target.value })} rows={4} placeholder={localizedFieldPlaceholder(lang, '主视觉副标题', 'Subtitle', 'de')} className="rounded-2xl border border-stone-200 px-4 py-3 text-sm outline-none focus:border-amber-500" />
          <textarea value={hero.subtitleEn ?? ''} onChange={(e) => setHero({ ...hero, subtitleEn: e.target.value })} rows={4} placeholder={localizedFieldPlaceholder(lang, '主视觉副标题', 'Subtitle', 'en')} className="rounded-2xl border border-stone-200 px-4 py-3 text-sm outline-none focus:border-amber-500" />
          <textarea value={hero.noteDe ?? ''} onChange={(e) => setHero({ ...hero, noteDe: e.target.value })} rows={3} placeholder={localizedFieldPlaceholder(lang, '图片补充说明', 'Image note', 'de')} className="rounded-2xl border border-stone-200 px-4 py-3 text-sm outline-none focus:border-amber-500" />
          <textarea value={hero.noteEn ?? ''} onChange={(e) => setHero({ ...hero, noteEn: e.target.value })} rows={3} placeholder={localizedFieldPlaceholder(lang, '图片补充说明', 'Image note', 'en')} className="rounded-2xl border border-stone-200 px-4 py-3 text-sm outline-none focus:border-amber-500" />
          <label className="flex flex-col gap-2 text-sm text-stone-700 md:col-span-2">
            <span>{t(lang, '主视觉图片链接', 'Hero image URL')}</span>
          ...
          {faqs.filter((item) => !item._delete).map((item, index) => (
            <div key={item.id} className="rounded-3xl border border-stone-100 bg-[linear-gradient(180deg,#fff_0%,#fcfbf9_100%)] p-5">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-stone-700">
                <label className="flex items-center gap-2"><span>{t(lang, '显示顺序', 'Sort')}</span><input value={String(item.sortOrder)} onChange={(e) => { const next = [...faqs]; next[index] = { ...item, sortOrder: Number(e.target.value) || 0 }; setFaqs(next) }} className="w-20 rounded-xl border border-stone-200 px-3 py-2 outline-none focus:border-amber-500" /></label>
                <label className="flex items-center gap-2"><input type="checkbox" checked={item.isActive} onChange={(e) => { const next = [...faqs]; next[index] = { ...item, isActive: e.target.checked }; setFaqs(next) }} />{t(lang, '启用显示', 'Active')}</label>
              </div>
            </div>
          ))}
        </div>
            ...
            <div key={item.id} className="rounded-3xl border border-stone-100 bg-[linear-gradient(180deg,#fff_0%,#fcfbf9_100%)] p-5">
              ...
              <div className="grid gap-4 md:grid-cols-[140px_1fr]">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <input value={item.altDe} onChange={(e) => { const next = [...gallery]; next[index] = { ...item, altDe: e.target.value }; setGallery(next) }} placeholder={localizedFieldPlaceholder(lang, '图片替代说明', 'Alt text', 'de')} className="rounded-2xl border border-stone-200 px-4 py-3 text-sm outline-none focus:border-amber-500" />
                  <input value={item.altEn} onChange={(e) => { const next = [...gallery]; next[index] = { ...item, altEn: e.target.value }; setGallery(next) }} placeholder={localizedFieldPlaceholder(lang, '图片替代说明', 'Alt text', 'en')} className="rounded-2xl border border-stone-200 px-4 py-3 text-sm outline-none focus:border-amber-500" />
                  <label className="flex flex-col gap-2 text-sm text-stone-700 md:col-span-2"><span>{t(lang, '图片链接', 'Image URL')}</span><input value={item.imageUrl} onChange={(e) => { const next = [...gallery]; next[index] = { ...item, imageUrl: e.target.value }; setGallery(next) }} placeholder={t(lang, '可保留上传后的 /uploads/ 路径，或填写外部图片地址', 'Keep the uploaded /uploads/ path or enter an external image URL')} className="rounded-2xl border border-stone-200 px-4 py-3 outline-none focus:border-amber-500" /></label>
                  <label className="flex items-center gap-2 text-sm text-stone-700"><span>{t(lang, '显示顺序', 'Sort')}</span><input value={String(item.sortOrder)} onChange={(e) => { const next = [...gallery]; next[index] = { ...item, sortOrder: Number(e.target.value) || 0 }; setGallery(next) }} className="w-20 rounded-xl border border-stone-200 px-3 py-2 outline-none focus:border-amber-500" /></label>
                  <div className="flex flex-wrap items-center gap-4 text-sm text-stone-700">
                    <label className="flex items-center gap-2"><input type="checkbox" checked={item.isActive} onChange={(e) => { const next = [...gallery]; next[index] = { ...item, isActive: e.target.checked }; setGallery(next) }} />{t(lang, '启用显示', 'Active')}</label>
