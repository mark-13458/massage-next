...
### 后台页面职责
- **仪表盘**：今日预约、待确认预约、本周预约、热门服务、安全与治理入口
- **预约管理**：查看、筛选、改状态、备注、异常预约处理、自助改约/取消链追踪
- **服务管理**：新增、编辑、上下架、排序、服务页 SEO 承载
- **网站内容**：首页文案、关于、联系、FAQ、SEO 信息、合规页面内容
- **图片管理**：上传、替换、排序、alt 文本
- **系统设置**：管理员账号、邮件配置、站点配置、安全设置、功能开关、隐私治理待办
...
- 后台会话超时与登录失败限制逐步增强
- 自助改约 / 取消必须走安全 token
- 预约防刷、防恶意预约、基础审计痕迹需逐步后台化
- 安全策略不得破坏公开页面抓取、robots/sitemap 与多语言 SEO
...
当前仓库已经不再是“非常早期骨架”，而是进入了**可运营网站系统雏形**阶段：
- 前台核心页面已落地（首页、服务、预约、关于、联系、图库）
- 基础组件与双语结构已成形
- 数据模型已从基础版扩展到支撑官网、预约、内容、图库、后台运营的雏形
- 后台已落地登录保护、预约管理、服务管理、内容管理
- 基础 SEO 文件已接入（robots / sitemap），但更细的 metadata / hreflang / LocalBusiness schema 仍待补完
- 上传链已打通至 MVP，可支持 Gallery 与 Hero 图片上传，但资源回收、校验、替换策略仍需继续收口

因此下一步不再是“从零重构”，而是：

> 顺着当前方向，把上传链、后台体验、部署联调与生产准备推进到可上线级别。
